var market = (function() {
  var myDataRef = new Firebase('https://boiling-heat-634.firebaseio.com/market');
  var tiles = {
    1 : { 'discount' : 0},
    2 : { 'discount' : 0},
    3 : { 'discount' : 0},
  ...
  function setUp(){
    ...
    $('.active').keypress(market.onEnter);

    market.ref.on('child_added', function(snapshot) {
        curTiles[snapshot.key()] = snapshot.val();
        market.displayUsers(snapshot);
      });

    market.ref.on('child_removed', function(snapshot) {
